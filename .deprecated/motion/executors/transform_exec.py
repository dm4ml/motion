...
class TransformVersion(object):
    ...
    async def inferMany(self, ids):
        ...
        if self.inferBatchExists:
            ...
            # all_results = {id: res for id, res in zip(ids, results)}
            all_results = {
                id: self.transform.infer(features[0])
                for id, features in zip(ids, all_features)
            }
...
class TransformExecutorV2(object):
    def __init__(self, transform, store, upstream_executors=[]):
        ...
        self.state_history = {}
        self.transform_dummy = transform()
        self.feature_fields = [
            f.name
            for f in _get_fields_from_type(self.transform_dummy.featureType)
        ]
        if self.transform_dummy.labelType:
            self.label_fields = [
                f.name
                for f in _get_fields_from_type(self.transform_dummy.labelType)
            ]
        else:
            self.label_fields = []
        if self.transform_dummy.returnType and dataclasses.is_dataclass(
            self.transform_dummy.returnType
        ):
            self.return_fields = [
                f.name
                for f in _get_fields_from_type(self.transform_dummy.returnType)
            ]
        else:
            self.return_fields = []
        ...
        self.max_staleness = (
            ...
            if hasattr(self.transform_dummy, "max_staleness")
            else 0
        )
        self.ignore_fit = (
            self.transform_dummy.ignore_fit
            if hasattr(self.transform_dummy, "ignore_fit")
            else False
        )
        if self.ignore_fit:
    ...
    async def fetchFeatures(self, ids, version):
        ...
        features = [
            self.transform_dummy.featureType(
                **{
                    k: v
